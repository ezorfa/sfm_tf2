...
def colorize(value, vmin=None, vmax=None, cmap=None):
    ...
    # quantize
    indices = tf.dtypes.cast(tf.round(value * 255), tf.int32)
...
def pixel2cam(depth, pixel_coords, intrinsics, is_homogeneous=True):
  ...
  inv =  (tf.compat.v1.matrix_inverse(intrinsics))
  
  cam_coords = tf.multiply(tf.linalg.matmul(inv, pixel_coords) , depth)
  if is_homogeneous:
