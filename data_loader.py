...
class DataLoader(object):
    ...
    def load_train_batch(self):
        ...
        list_ds = tf.data.Dataset.from_tensor_slices(file_list['image_file_list'])
        ...
        def process_path(file_path):
          raw_cam_contents = tf.io.read_file(file_path + '_cam.txt')
          rec_def = []
          for i in range(9):
              rec_def.append([1.])
          ...
          image_seq = decode_img(img)
          tgt_image, src_image_stack = self.unpack_image_sequence (image_seq, self.img_height, self.img_width, self.num_source)
          
          return src_image_stack, tgt_image, intrinsics
          
        combined_ds = list_ds.shuffle(45000, seed=seed).map(process_path, num_parallel_calls=tf.data.experimental.AUTOTUNE)

        combined_ds = combined_ds.batch(self.batch_size,drop_remainder=True)

        combined_ds = combined_ds.map(self.prepare_data_augmentation, num_parallel_calls=tf.data.experimental.AUTOTUNE)

        combined_ds = combined_ds.prefetch(buffer_size=tf.data.experimental.AUTOTUNE)
        
        return combined_ds    
    
    def prepare_data_augmentation(self,src_image_stack, tgt_image, intrinsics):
    ...
    def make_intrinsics_matrix(self, fx, fy, cx, cy):
        ...
        r3 = tf.tile(r3, tf.convert_to_tensor([self.batch_size, 1]))
